...
# Build time
unless envBlank?(ENV["BITRISE_BUILD_TRIGGER_TIMESTAMP"])
  build_start_timestamp = ENV['BITRISE_BUILD_TRIGGER_TIMESTAMP']
  current_timestamp = Time.now.to_i

  build_time = current_timestamp.to_i - build_start_timestamp.to_i
  message "Build time: " + (build_time / 60).to_i.to_s + "min"
end
...
unless envBlank?(ENV["FRANCIS_PROJECT_ID"])
  ...
  mobiledevops.request_url = ENV["FRANCIS_BASE_URL"]
  mobiledevops_json = {
    "project_id": ENV["FRANCIS_PROJECT_ID"],
